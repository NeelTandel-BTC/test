class TvShowsController < ApplicationController
  ...
  # GET /tv_shows.json
  def index
    @q = TvShow.ransack(params[:q])
    @tv_shows = @q.result(distinct: true).includes(:channel)
  end
  # GET /tv_shows/1
  # GET /tv_shows/1.json
  def show
  ...
  def like_unlike
    show_id = params.dig(:tv_show_id)
    show = TvShow.find_by(id: show_id)
    if current_user.favorite_show_ids.include?(show_id.to_i)
